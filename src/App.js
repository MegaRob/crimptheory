...
import { getFirestore, collection, addDoc, getDocs, query, serverTimestamp, doc, setDoc, getDoc } from 'firebase/firestore'; // Added doc, setDoc, getDoc for profile management

// Global variables provided by the Canvas environment
// These are typically injected by the hosting environment (like Canvas or a CI/CD pipeline)
...
const LoginPage = ({ onLoginSuccess, onNavigateToRegister }) => {
  ...
  return (
    // Added bg-red-500 for testing Tailwind CSS functionality
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-white p-4 bg-red-500">
      <h1 className="text-4xl font-extrabold mb-8 text-center leading-tight">Login to Crimp Theory</h1> {/* CHANGED TEXT HERE */}
...
const RegisterPage = ({ onRegisterSuccess, onNavigateToLogin }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-white p-4">
      <h1 className="text-4xl font-extrabold mb-8 text-center leading-tight">Register for Crimp Theory</h1> {/* CHANGED TEXT HERE */}
      <div className="bg-gray-800 p-8 rounded-lg shadow-xl border border-gray-700 w-full max-w-md flex flex-col items-center space-y-6">
        <input
          type="email"
          placeholder="Email"
...
const ProfilePage = ({ onBackToHome, userId, db }) => {
  const [firstName, setFirstName] = useState('');
  const [lastName, setLastName] = useState('');
  const [currentWeight, setCurrentWeight] = useState('');
  const [maxLiftWeight, setMaxLiftWeight] = useState('');
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      setLoading(true);
      try {
        // Path: /artifacts/{appId}/users/{userId}/profile/userProfile
        ...
        if (docSnap.exists()) {
          ...
          setMaxLiftWeight(data.maxLiftWeight || '');
        }
      } catch (error) {
        console.error("Error fetching profile:", error);
        setMessageBox(`Error loading profile: ${error.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSaveProfile = async () => {
    if (!userId) {
      setMessageBox("Error: User not authenticated. Cannot save profile.");
      return;
    }
    setLoading(true);
    try {
      const profileData = {
        firstName: firstName,
        lastName: lastName,
        currentWeight: parseFloat(currentWeight) || null, // Convert to number
        maxLiftWeight: parseFloat(maxLiftWeight) || null, // Convert to number
        lastUpdated: serverTimestamp(),
      };
      const profileDocRef = doc(db, `artifacts/${appId}/users/${userId}/profile`, 'userProfile');
      await setDoc(profileDocRef, profileData, { merge: true }); // Use merge to update existing fields
      setMessageBox("Profile saved successfully!");
    } catch (error) {
      console.error("Error saving profile:", error);
      setMessageBox(`Error saving profile: ${error.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-white p-6 flex flex-col items-center">
      ...
      <h1 className="text-4xl font-extrabold mb-8 text-center text-blue-400">Edit Profile</h1>
        ...
        {loading ? (
          ...
          <>
            <div className="w-full">
              <label htmlFor="firstName" className="block text-lg font-semibold mb-2 text-gray-300">First Name:</label>
              <input
                id="firstName"
                type="text"
                value={firstName}
            ...
            <div className="w-full">
              <label htmlFor="lastName" className="block text-lg font-semibold mb-2 text-gray-300">Last Name:</label>
              <input
                id="lastName"
                type="text"
                value={lastName}
            ...
            <div className="w-full">
              <label htmlFor="currentWeight" className="block text-lg font-semibold mb-2 text-gray-300">Current Weight (lbs/kg):</label>
              <input
                id="currentWeight"
                type="number"
                value={currentWeight}
            ...
            <div className="w-full">
              <label htmlFor="maxLiftWeight" className="block text-lg font-semibold mb-2 text-gray-300">Max Lift Weight (lbs/kg):</label>
              <input
                id="maxLiftWeight"
                type="number"
                value={maxLiftWeight}
                onChange={(e) => setMaxLiftWeight(e.target.value)}
                className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-md text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="e.g., 25"
                min="0"
              />
...
const Home = ({ onSelectProtocol, onViewHistory, userEmail, onLogout, onEditProfile }) => { // Added onEditProfile
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-white p-4">
      <h1 className="text-4xl font-extrabold mb-8 text-center leading-tight">Crimp Theory</h1> {/* CHANGED TEXT HERE */}
      ...
      <div className="bg-gray-800 p-8 rounded-lg shadow-xl border border-gray-700 w-full max-w-md flex flex-col items-center space-y-6">
        ...
        <div className="grid grid-cols-1 gap-4 w-full"> {/* Adjusted grid to single column */}
          ...
          <button
            onClick={() => handleStart('maximalForce')} // New button for Maximal Force
            className="bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-5 rounded-lg shadow-md transform transition duration-300 ease-in-out hover:scale-105 focus:outline-none focus:ring-4 focus:ring-red-500 focus:ring-opacity-75"
          >
            Maximal Force Protocol
          </button>
          <button
          ...
          <button
            onClick={onEditProfile} // New button to edit profile
            className="bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 px-5 rounded-lg shadow-md transform transition duration-300 ease-in-out hover:scale-105 focus:outline-none focus:ring-4 focus:ring-purple-500 focus:ring-opacity-75 col-span-full"
          >
            Edit Profile
          </button>
          <button
...
const ComplexRepeaterProtocol = ({ onBackToHome, weight, userId, db }) => {
  ...
  // State to hold the last work step's details for display during rest
  const [lastWorkStepDetails, setLastWorkStepDetails] = useState(null);
  ...
  const handleGripType1Select = (gripType) => {
    ...
    setMessage("Ready"); // CHANGED TEXT HERE
    if (plan1.length > 0) {
  ...
  const resetWorkout = () => {
    ...
    setLastWorkStepDetails(null); // Reset last work step details
  };
  ...
  useEffect(() => {
    if (workoutRunning && currentTime > 0) {
      ...
      if (currentStep) {
          ...
          // Update lastWorkStepDetails if the just-completed step was a work step
          if (currentStep.type === 'work') {
            setLastWorkStepDetails({
              grip: currentStep.grip,
              hand: currentStep.hand,
              setNumber: currentStep.setNumber,
              liftNumber: currentStep.liftNumber,
              actualWeight: parseFloat(getActualWeight(weight, currentStep.intensity)),
            });
          }
      }
  ...
  // Determine which details to display: current step's or last work step's
  const displayDetails = isWorkPhase ? currentStep : lastWorkStepDetails;
  const displayWeight = displayDetails && displayDetails.intensity ? getActualWeight(weight, displayDetails.intensity) : (displayDetails && displayDetails.actualWeight !== undefined ? displayDetails.actualWeight : null);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-white p-6 flex flex-col items-center justify-center"> {/* Added flex-col items-center justify-center */}
      <button
      ...
      <div className="bg-gray-800 p-8 rounded-lg shadow-xl border border-gray-700 w-full max-w-md flex flex-col items-center space-y-6">
        ...
        {/* Display workout details based on displayDetails state */}
        {displayDetails && (
          <>
            {displayDetails.grip && (
                <p className="text-xl text-gray-300">
                    Grip: <span className="font-bold text-teal-400">{displayDetails.grip}</span>
                </p>
            )}
            {displayDetails.hand && (displayDetails.hand !== 'Both Hands') && (
                <p className="text-xl text-gray-300">
                    Hand: <span className="font-bold text-teal-400">{displayDetails.hand}</span>
                </p>
            )}
            {displayDetails.setNumber && (
                <p className="text-xl text-gray-300">
                    Set: <span className="font-bold text-teal-400">{displayDetails.setNumber}</span> / 3
                </p>
            )}
            {displayDetails.liftNumber && ( // Always show lift number if it exists
                <p className="text-xl text-gray-300">
                    Lift: <span className="font-bold text-teal-400">{displayDetails.liftNumber}</span> / 6
                </p>
            )}
            {displayWeight !== null && (
              <p className="text-xl text-gray-300">
...
// MaximalForceProtocol component
const MaximalForceProtocol = ({ onBackToHome, weight, userId, db }) => {
  // Function to create a workout plan for a single grip type and both hands
  const createMaximalForceWorkoutPlan = (gripType) => {
    const sets = [];
    const holdDuration = 8; // 7-10 seconds, using 8s
    const restBetweenLifts = 150; // 2-3 minutes (2.5 minutes)
    const totalSetsPerHand = 5;
    const liftsPerSet = 1;

    const currentIntensity = 1.0; // All sets are working sets at target weight

    const addHandSets = (hand) => {
      for (let setIdx = 0; setIdx < totalSetsPerHand; setIdx++) {
        for (let liftIdx = 1; liftIdx <= liftsPerSet; liftIdx++) {
          sets.push({
            id: `${gripType}-${hand}-Set${setIdx + 1}-Lift${liftIdx}`,
            type: 'work',
            duration: holdDuration,
        ...
        // Add rest between sets, if not the very last set for this hand
        if (setIdx < totalSetsPerHand - 1) {
          sets.push({
            id: `${gripType}-${hand}-Set${setIdx + 1}-Rest`,
            type: 'rest',
            duration: restBetweenLifts,
            grip: gripType,
            hand: hand,
            subType: 'Rest',
            setGroup: 'Working Set',
    ...
    sets.push({
      id: `${gripType}-Transition-Hand`,
      type: 'rest',
      duration: 300, // 5 minutes rest between hands
      grip: gripType,
      hand: 'Both Hands',
      subType: 'Rest Between Hands',
      message: "Rest"
  ...
  const [selectedGripType1, setSelectedGripType1] = useState(null);
  const [selectedGripType2, setSelectedGripType2] = useState(null); // Fix: Corrected typo
  const [workoutPlan, setWorkoutPlan] = useState([]);
  const [currentStepIndex, setCurrentStepIndex] = useState(0);
  const [workoutRunning, setWorkoutRunning] = useState(false);
  const [currentTime, setCurrentTime] = useState(0);
  const [currentRPE, setCurrentRPE] = useState(0);
  const [isWaitingForRPE, setIsWaitingForRPE] = useState(false);
  const [workoutSummaryData, setWorkoutSummaryData] = useState([]);
  const [messageBox, setMessageBox] = useState(null);
  const [message, setMessage] = useState("Select your first grip type for Maximal Force Protocol.");

  const timerRef = useRef(null);
  const workoutStartTime = useRef(0);
  const [lastWorkStepDetails, setLastWorkStepDetails] = useState(null); // To keep info during rest
  ...
  const handleGripType1Select = (gripType) => {
    ...
    const plan1 = createMaximalForceWorkoutPlan(gripType);
    const transitionRest = [{
      id: `Overall-Transition-GripTypes`,
      type: 'rest',
      duration: 300,
    ...
    const plan2 = createMaximalForceWorkoutPlan(otherGripType);
  ...
  const saveMaximalForceSummaryToFirestore = useCallback(async (finalRPE, fullSummary, totalLiftTime, totalRestTime, totalWorkoutDuration) => {
    if (!userId) {
      setMessageBox("Error: User not authenticated. Cannot save workout.");
      return;
    }
    try {
      const protocolTitle = `Maximal Force Protocol (${selectedGripType1 || 'N/A'} then ${selectedGripType2 || 'N/A'})`;
      const workoutCollectionRef = collection(db, `artifacts/${appId}/users/${userId}/workoutSummaries`);
      await addDoc(workoutCollectionRef, {
        protocol: protocolTitle,
        weight: weight,
      ...
      console.log("Maximal Force workout summary saved to Firestore.");
      setMessageBox("Workout saved successfully!");
    } catch (error) {
      console.error("Error saving Maximal Force summary to Firestore:", error);
      setMessageBox(`Error saving workout: ${error.message}`);
    }
  }, [userId, db, appId, selectedGripType1, selectedGripType2, weight]);
  ...
  const resetWorkout = () => {
    ...
    setSelectedGripType2(null);
    setWorkoutPlan([]);
    setMessage("Select your first grip type for Maximal Force Protocol.");
    workoutStartTime.current = 0;
    setLastWorkStepDetails(null);
  };
  ...
  const handleFinalRPEConfirm = useCallback(() => {
    ...
    saveMaximalForceSummaryToFirestore(currentRPE, workoutSummaryData, totalLiftTime, totalRestTime, totalWorkoutDurationPlanned);
    setCurrentRPE(0);
    setMessage("Workout complete!");
  }, [currentRPE, workoutSummaryData, saveMaximalForceSummaryToFirestore, workoutPlan]);
  ...
  useEffect(() => {
    ...
    return () => clearInterval(timerRef.current);
  }, [workoutRunning, currentTime, currentStepIndex, workoutPlan, workoutSummaryData, saveMaximalForceSummaryToFirestore, weight, currentStep]);
  ...
  if (!selectedGripType1) {
      return (
          <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-white p-6 flex flex-col items-center justify-center space-y-6">
              <h2 className="text-3xl font-bold text-teal-300 mb-4">Select Maximal Force Grip Types</h2>
              <p className="text-lg text-gray-300 text-center mb-4">Choose the first grip type for your workout. The other will be used second.</p>
              <button
                onClick={() => handleGripType1Select('20mm Half-Crimp')}
                className="px-8 py-4 bg-blue-600 text-white font-bold rounded-lg shadow-md hover:bg-blue-700 transition duration-300 ease-in-out hover:scale-105 focus:outline-none focus:ring-4 focus:ring-blue-500 focus:ring-opacity-75 w-64"
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-white p-6 flex flex-col items-center justify-center">
      ...
      <h1 className="text-4xl font-extrabold mb-8 text-center text-blue-400">Maximal Force Protocol</h1>
      <h2 className="text-2xl font-bold mb-4 text-center text-teal-300">
        ({selectedGripType1} then {selectedGripType2})
      </h2>
      ...
      <div className="bg-gray-800 p-8 rounded-lg shadow-xl border border-gray-700 w-full max-w-md flex flex-col items-center space-y-6">
        ...
        {displayDetails && (
          <>
            ...
            {displayDetails.setNumber && (
                <p className="text-xl text-gray-300">
                    Set: <span className="font-bold text-teal-400">{displayDetails.setNumber}</span> / 5
                </p>
            )}
            {displayDetails.liftNumber && (
                <p className="text-xl text-gray-300">
                    Lift: <span className="font-bold text-teal-400">{displayDetails.liftNumber}</span> / 1
                </p>
            )}
            {displayWeight !== null && (
              <p className="text-xl text-gray-300">
...
const App = () => {
  ...
  const handleSelectProtocol = (protocol, selectedWeight) => {
    ...
    // Request wake lock when a workout starts
    requestWakeLock();
  };

  const handleBackToHome = () => {
    setCurrentView('home');
    // Release wake lock when returning to home
    releaseWakeLock();
  };


  // Pass userEmail to Home component
  const commonProps = { onBackToHome: handleBackToHome, userId, db, weight };
  ...
  switch (currentView) {
    case 'home':
      return <Home onSelectProtocol={handleSelectProtocol} onViewHistory={() => setCurrentView('history')} userId={userId} userEmail={userEmail} onLogout={handleLogout} onEditProfile={() => setCurrentView('profile')} />;
    case 'repeaters':
      return <ComplexRepeaterProtocol {...commonProps} />;
    case 'maximalForce': // New case for Maximal Force protocol
      return <MaximalForceProtocol {...commonProps} />;
    case 'history':
    ...
    default:
      return <Home onSelectProtocol={handleSelectProtocol} onViewHistory={() => setCurrentView('history')} userId={userId} onLogout={handleLogout} onEditProfile={() => setCurrentView('profile')} />;
  }
};
